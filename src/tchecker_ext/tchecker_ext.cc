...
void usage(std::string const & exec_name)
{
  ...
  std::cerr << "        explore_ext       run explore_ext algorithm" << std::endl;
  std::cerr << "        explore_ext       run explore_ext algorithm" << std::endl;
...
//      break;
    case COMMAND_COVREACH:
    {
      ...
      break;
    case COMMAND_COVREACH_EXT:
    {
      std::tie(map, index) = parse_options(argc-1, argv+1, tchecker_ext::covreach_ext::options_t::getopt_long_options,
                                           tchecker_ext::covreach_ext::options_t::getopt_long_options_long, log);
      ++index;    // accounts for argv+1 above
    }
      break;
    default:
...
//        break;
      case COMMAND_COVREACH:
        ...
        break;
      case COMMAND_COVREACH_EXT:
        tchecker_ext::covreach_ext::options_t::describe(std::cerr);
        break;
      default:
        break;
    }
...
//        break;
      ...
      case COMMAND_COVREACH_EXT:
      {
        tchecker_ext::covreach_ext::options_t options(tchecker::make_range(map.begin(), map.end()), log);
        tchecker_ext::covreach_ext::run(*sysdecl, options, log);
      }
        break;
      default:
        break;
